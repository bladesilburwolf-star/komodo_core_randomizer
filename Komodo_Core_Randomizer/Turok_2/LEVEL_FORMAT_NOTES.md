...
## Section 4 — particles (in-game trial, 2026-09-13)

Safe to scramble; game **boots** with heavy side effects. Observed when experimentally shuffled:

| Effect | Notes |
|--------|--------|
| Enemy weapons disabled | Hostiles may not fire / reduced offense |
| Weaker enemies | Easier fights overall |
| Talon / claw stronger | Player melee feels boosted |
| Invisible portals | Warp/portal visuals missing (function may remain) |
| Blood disabled | Gore FX off |
| Ammo FX / pickups presentation broken | Ammo-related particles gone |
| Gun sounds disabled | Some weapon audio missing |
| **+10–15 FPS** | Particle load drop helps performance |

**Randomizer use:** good **toybox / chaos** toggle — not a pure difficulty slider (buffs player melee while nerfing enemy offense and stripping FX). Label in UI as **Particles chaos** rather than generic experimental.

Does **not** appear to softlock boot. Prefer enabling alone when testing so effects are attributable.
